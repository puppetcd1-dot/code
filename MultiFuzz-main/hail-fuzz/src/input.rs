...
pub struct MultiStream {
    ...
    pub read_context: Option<ReadContext>,
}
...
impl Clone for MultiStream {
    ...
    fn clone_from(&mut self, source: &Self) {
        ...
        self.read_context = source.read_context;
    }
}

impl MultiStream {
    pub fn new(streams: HashMap<StreamKey, StreamData>) -> Self {
        Self { streams, last_read: None, tracer: None, read_context: None }
    }
    ...
    pub fn next_bytes(&mut self, addr: StreamKey, size: usize) -> Option<&[u8]> {
        ...
        let input_offset = stream.cursor;
        let buf = stream.bytes.get(stream.cursor as usize..stream.cursor as usize + size)?;
        if let Some(tracer) = self.tracer.as_mut() {
            tracer.read(addr, input_offset, buf, self.read_context.unwrap_or_default());
        }
    ...
    pub fn set_read_context(&mut self, context: ReadContext) {
    ...
    pub fn from_bytes(buf: &[u8]) -> Option<Self> {
        ...
        Some(MultiStream { streams, last_read: None, tracer: None, read_context: None })
    }
...
mod legacy {
    ...
    pub fn multi_stream_from_bytes_v0(buf: &[u8]) -> Option<super::MultiStream> {
        ...
        Some(super::MultiStream { streams, last_read: None, tracer: None, read_context: None })
    }
}
...
impl ReadContextSource for MultiStream {
