...
///the max stream len of the input.
...
pub(crate) fn extend_input_by_rand<R: Rng>(
    ...
    let remaining = MAX_STREAM_LEN.saturating_sub(input.len());

    // Small chance try a large extension up to the maximum limit.
    if rng.gen_bool(0.001) {
        let amount = extension_limit.min(remaining);
        return extend_input_by(rng, dict, input, amount);
    }
    ...
    let amount = base.min(max_extension).min(extension_limit).min(remaining);
...
pub(crate) fn extend_input_by<R: Rng>(
    ...
    let remaining = MAX_STREAM_LEN.saturating_sub(bytes.len());
    let amount = amount.min(remaining);

    // 如果已无扩展空间，直接返回，避免任何分配
    if amount == 0 {
        return Extension::Random; // 或返回一个表示未扩展的标记
    }
